...
describe('ServiceType Service', () => {
      ...
      const mockServiceTypes = [
        {
          ...
          description: 'desc1',
          duration: 30,
          especialidad: { id: 1, name: 'Odontología', description: 'desc', is_active: true },
          is_active: true
        },
      ];
  ...
  describe('getByEspecialidad', () => {
    it('debería obtener tipos de servicio por especialidad', async () => {
      ...
      ServiceType.findAll.mockResolvedValue(mockServiceTypes);
      const result = await serviceTypeService.getByEspecialidad(1);
      expect(ServiceType.findAll).toHaveBeenCalled();
      expect(result).toEqual(ServiceTypeOutputDto.fromList(mockServiceTypes));
    });
  });
  ...
  describe('getById', () => {
    ...
    it('debería lanzar error si no encuentra el tipo de servicio', async () => {
      ...
      await expect(serviceTypeService.getById(99)).rejects.toThrow('Tipo de servicio no encontrado');
    });
  });
  ...
  describe('create', () => {
    ...
    it('debería lanzar error si falta el nombre', async () => {
      await expect(serviceTypeService.create({ duration: 30, especialidad_id: 1 }))
        .rejects.toThrow('El nombre del servicio es requerido y debe tener al menos 2 caracteres');
    });
    it('debería lanzar error si el nombre está duplicado', async () => {
      Especialidad.findOne.mockResolvedValue({ id: 1, name: 'Odontología', is_active: true });
      ServiceType.findOne.mockResolvedValue({ id: 1, name: 'Existente' });
      await expect(serviceTypeService.create({ name: 'Existente', duration: 30, especialidad_id: 1 }))
        .rejects.toThrow('Ya existe un servicio con este nombre en esta especialidad');
    });
  });

  describe('update', () => {
    it('debería actualizar tipo de servicio exitosamente', async () => {
      const existingServiceType = { id: 1, name: 'Viejo', duration: 30, especialidad_id: 1, is_active: true, update: jest.fn() };
      const updateData = { name: 'Nuevo', description: 'nueva desc', duration: 45, especialidad_id: 1 };
      const updatedServiceType = { id: 1, name: 'Nuevo', description: 'nueva desc', duration: 45, especialidad_id: 1, is_active: true, especialidad: { id: 1, name: 'Odontología', description: 'desc', is_active: true } };
      ServiceType.findOne
    ...
    it('debería lanzar error si no encuentra el tipo de servicio', async () => {
      ServiceType.findOne.mockResolvedValue(null);
      await expect(serviceTypeService.update(99, { name: 'Nuevo', duration: 30, especialidad_id: 1 }))
    ...
    it('debería lanzar error si el nombre está duplicado', async () => {
      ...
      Especialidad.findOne.mockResolvedValue({ id: 1, name: 'Odontología', is_active: true });
      await expect(serviceTypeService.update(1, { name: 'Duplicado', duration: 30, especialidad_id: 1 }))
        .rejects.toThrow('Ya existe un servicio con este nombre en esta especialidad');
    });
  });
  ...
  describe('deactivate', () => {
    ...
    it('debería lanzar error si no encuentra el tipo de servicio', async () => {
      ServiceType.findOne.mockResolvedValue(null);
      await expect(serviceTypeService.deactivate(99)).rejects.toThrow('Tipo de servicio no encontrado');
    });
  });
}); 